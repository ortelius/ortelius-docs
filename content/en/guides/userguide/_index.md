...
> **Building custom tooling?** `POST /api/v1/releases` also accepts an SBOM directly if you're integrating with Ortelius programmatically — see the [Implementation Guide](implementation/) — but this isn't something you need to add to your CI/CD pipeline for normal use; the scanner components above cover that.
